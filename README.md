...
浏览器打开 **http://localhost:8080**，默认进入**规则管理页**（首次启动自动初始化 5 条案例规则）：
...
├── web/                   # Blockly 前端
│   ├── index.html         # 规则管理页（默认首页）
│   ├── editor.html        # 规则编辑器（Blockly 拖拽）
│   ├── js/                # 自定义积木 / Lua 生成器 / 管理页与编辑器逻辑
